(function( $ ) {
	...
	$(function(){
		...
		//remove all button
		$( "#srfw_removeallbtn" ).click(function() {
		  openpopup("Are you sure?", '<p>This will delete all reviews in your Wordpress database including the ones you manually entered. It Does NOT affect your reviews on Facebook.</p>', '<a class="button dashicons-before dashicons-no" href="?page=srfw-reviews&opt=delall">Remove</a>');
		});	

		//upgrade to pro
		$( ".srfw_upgrade_needed" ).click(function() {
		  openpopup("Upgrade Needed", '<p>Please upgrade to the Pro Version of this Plugin to access this feature.</p>', '<a class="button dashicons-before  dashicons-cart" href="?page=srfw-get_pro">Upgrade Here</a>');
		});		
		...
		$( "#srfw_addnewreviewbtn" ).click(function() {
		  jQuery("#srfw_new_review").show("slow");
		});	
		$( "#srfw_addnewreview_cancel" ).click(function() {
		  jQuery("#srfw_new_review").hide("slow");
		  //reload page without taction and tid
		  setTimeout(function(){ 
			window.location.href = "?page=srfw-reviews"; 
		  }, 500);
		...
		$('#upload_avatar_button').click(function() {
			tb_show('Upload Reviewer Avatar', 'media-upload.php?referer=srfw-reviews&type=image&TB_iframe=true&post_id=0', false);
			return false;
		});
		...
		window.send_to_editor = function(html) {
			...
			$('#srfw_nr_avatar_url').val(image_url);
		...
		$("#newreviewform").submit(function(){ 
			  ...
			  if ($('input[name=srfw_nr_rating]:checked').length) {
		...
		$("#review_list").on("click", ".reveditbtn", function (event) {
			...
			$("#srfw_new_review").show("slow");
			//find values from rowobject and fill in edit form srfw_nr_rating, srfw_nr_text, srfw_nr_name, srfw_nr_avatar_url, srfw_nr_date
			$("#editrid").val(rid);
			$("#srfw_nr_name").val(name);
			$("#srfw_nr_avatar_url").val(wprev_row_userpic);
			//for radio
			$("#srfw_nr_rating").val(wprev_row_rating);
			$("#srfw_nr_date").val(wprev_row_created_time);
			
			$("#srfw_nr_text").val(wprev_row_review_text);
		...
		function sendtoajax(rid,whattodo,rowobject){
			var senddata = {
					action: 'srfw_hide_review',	//required
		...
		//for pagination bar-----------------------------------
		$("#srfw_review_list_pagination_bar").on("click", "span", function (event) {
		...
		function sendtoajaxreview(pageclicked,sortbyval,sortd,selrevs){
			...
			$( "#review_list" ).html("");
			$( "#srfw_review_list_pagination_bar" ).html("");
			var senddata = {
				...
				var reviewtext = '';
					...
					$.each(object, function(index) {
						...
						if(object[index].reviewer_name){
							...
							if(object[index].type=="Facebook"){
								...
								editdellink ='';
							} else if(object[index].type=="Zillow"){
					...
					if(numpages>1){
						...
						$( "#srfw_review_list_pagination_bar" ).html(pagebarhtml);

					if(reviewtotalcount==0){
						$("#srfw_review_list_pagination_bar").hide();
					} else {

						$("#srfw_review_list_pagination_bar").show();
					}
